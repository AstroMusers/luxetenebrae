...
Parameters:
- number_of_runs: int, number of COMPAS runs to execute.
- mode: str, mode for the runs (e.g., 'WD_Enabled_Detailed').
"""
...
print(f"File directory: {HERE}")
print(f"Current directory: {os.getcwd()}")
REPO_ROOT = os.path.abspath(os.path.join(HERE, "../../"))
print(f"Repository root: {REPO_ROOT}")

DEFAULT_CONFIG_FILE = os.path.join(REPO_ROOT, 'config/my_compasConfigDefault.yaml')
print(f"Default config file: {DEFAULT_CONFIG_FILE}")

if not os.path.exists(os.path.join(REPO_ROOT, "runs", mode)):
    os.makedirs(os.path.join(REPO_ROOT, "runs", mode))
os.environ["COMPAS_LOGS_OUTPUT_DIR_PATH"] = os.path.join(REPO_ROOT, "runs", mode) # This is the default output directory for COMPAS logs
print(f"Output directory: {os.environ['COMPAS_LOGS_OUTPUT_DIR_PATH']}")
for i in range(number_of_runs):

    print(f"Running COMPAS simulation {i+1}/{number_of_runs} in mode '{mode}'")
    class pythonProgramOptions:
        """
        A class to store and access COMPAS program options in python
        """
        print("Initializing pythonProgramOptions...")
        def __init__(self, config_file=DEFAULT_CONFIG_FILE, grid_filename=None,
            ...
            print('compas_root_dir', compas_root_dir)
